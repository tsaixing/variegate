  ...
  try {
    ...
    if (plantKeyMap.length) {
      return plantKeyMap
        ...
        .reduce((accumulator, value) => (accumulator[value.plantKey] = value, accumulator), {})
    }
  } catch (err) {
    console.log(err);
  }
...
export async function getStatusMapData() {
  try {
    ...
    if (statusMap.length) {
      return statusMap
        .map((row) => ({
          spreadsheetStatus: row[0].trim(),
          siteStatus: row[1].trim(),
        }))
        .reduce((accumulator, value) => (accumulator[value.spreadsheetStatus] = value, accumulator), {})
    }
  } catch (err) {
    console.log(err);
  }
